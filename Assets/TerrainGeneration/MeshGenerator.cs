...
namespace TerrainProceduralGenerator
{
    ...
    {
        public static MeshData GenerationTerrainMesh(float[,] heightMap, MeshSettings meshSettings, int levelOfDetail)
        {
            int skipIncrement = (levelOfDetail == 0) ? 1 : levelOfDetail * 2;

            int numVertsPerLine = meshSettings.numVertsPerLine;

            Vector2 topLeft = new Vector2(-1, 1) * meshSettings.meshWorldsSize / 2f;
                ...
                for (int x = 0; x < numVertsPerLine; x++)
                {
                    ...
                    if (!isSkippedVertex)
                    {
                        ...
                        Vector2 vertexPosition2D =
                            topLeft + new Vector2(percent.x, -percent.y) * meshSettings.meshWorldsSize;
    ...
    public class MeshData
    {
        ...
        private Vector3[] outOfMeshVerteces;
        ...
        public MeshData(int numVerticesPerLine, int skipIncrement, bool useFlatShading)
        {
            ...
            int numMainTriangles = (numVerticesPerLine - 1) * (numVerticesPerLine - 1) * 2;
            triangles = new int[(numMeshEdgeTriangles + numMainTriangles) * 3];

            outOfMeshVerteces = new Vector3[numVerticesPerLine * 4 - 4];
        ...
        public void AddVertex(Vector3 vertexPosition, Vector2 uv, int vertexIndex)
        {
            if (vertexIndex < 0)
            {
                outOfMeshVerteces[-vertexIndex - 1] = vertexPosition;
            }
            else
            {
        ...
        {
            Vector3 pointA = (indexA < 0) ? outOfMeshVerteces[-indexA - 1] : vertices[indexA];
            Vector3 pointB = (indexB < 0) ? outOfMeshVerteces[-indexB - 1] : vertices[indexB];
            Vector3 pointC = (indexC < 0) ? outOfMeshVerteces[-indexC - 1] : vertices[indexC];
        ...
        void FlatShading()
        {
            ...
            Vector2[] flatShadedUcs = new Vector2[triangles.Length];
            for (int i = 0; i < triangles.Length; i++)
            {
                flatShadedVertices[i] = vertices[triangles[i]];
                flatShadedUcs[i] = UVs[triangles[i]];
                triangles[i] = i;
            }
